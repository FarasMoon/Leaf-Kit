...
/** Track whether the article editor has unsaved changes */
...
async function loadArticles() {
  ...
  try {
    ...
    // Parallel loading of all article files
...
function parseArticleFrontMatter(content) {
  ...
  // Multi-line value support: if a line starts with whitespace, append to previous value
  ...
  for (const line of lines) {
    // Indented continuation of multi-line value (YAML block scalar style)
...
function renderArticleTable() {
  ...
  // Use DocumentFragment for better performance with large article lists
...
// Mark article as dirty on any front matter or content change
...
async function saveArticle(silent) {
  ...
  try {
    ...
    if (!silent) {
      ...
      // Silent save: update current path if new article
...
// Editor content preview + auto-save
let articleAutoSaveTimer = null;
let articleAutoSaveStatus = null;

// Listen for content changes to trigger preview update and auto-save
...
// Mark dirty on front matter field changes
